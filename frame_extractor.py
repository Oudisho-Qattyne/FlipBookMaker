...
class FrameExtractor:
    ...
    def extract_frames(self):
        ...
        A4 = cv2.imread('A4.jpg')
        A4_width = A4.shape[1]
        A4_height = A4.shape[0]
        # orig_file_dir = self.create_dir_if_not_exists('orig_size_frames')
        resize_file_dir = self.create_dir_if_not_exists('re_size_frames' + vidname)
        while (vid_cap.isOpened()):
            success, image = vid_cap.read()
            image_width  = vid_cap.get(3)
            image_height = vid_cap.get(4)
            scale = A4_width / (2 * image_width)
            scaleRotate = A4_width / (2 * image_height)
            print(scale)
            
            dim = (int(image_width * scale) , int(image_height * scale))
            dimRotate = ( int(image_height * scaleRotate) , int(image_width * scaleRotate))
            if success:
                try:
                    # orig_file_location = f"{orig_file_dir}/{vidname}_{count}.{self.img_frmt}"
                    resize_file_location = f"{resize_file_dir}/{count}.{self.img_frmt}"
